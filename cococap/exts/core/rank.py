...
class Ranks(commands.Cog):
    ...
    async def rank(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title=f"Current Rank: *{user_rank.display_name}* {ranks[user.rank]['emoji']}",
        ...
        user_rank = 0
        try:
            for index, user_rank in enumerate(ranks):
                if user_rank == user.rank:
            ...
            if rank == user_rank:
                ...
                if index == len(ranks):
                    ...
                    rankup_embed.add_field(
                        name="Wage", value=f"{'{:,}'.format(next_rank['wage'])} bits"
                    )
                ...
                role_to_remove = discord.utils.get(
                    interaction.guild.roles, name=user_rank.display_name
                )
                break

        if user.get_field("tokens") >= next_rank["price"]:
            ...
            class RolePurchaseButtons(discord.ui.View):
                ...
                @discord.ui.button(
                    label=f"{next_rank['price']:,} tokens", style=discord.ButtonStyle.green
                )
                async def green_button(
                    self, green_button_interaction: discord.Interaction, button: discord.ui.Button
                ):
                    if green_button_interaction.user != interaction.user:
                        ...
                        purchased_embed.set_footer(
                            text=f"User: {green_button_interaction.user.name}"
                        )
                        ...
                        await user.inc_tokens(tokens=-next_rank["price"])
                ...
                async def cancel_button(
                    ...
                    cancel_embed = discord.Embed(
                        title=f"Cancelled rankup to {next_rank_name} " f"{next_rank['emoji']}",
            ...
            class RolePurchaseButtons(discord.ui.View):
                ...
                @discord.ui.button(
                    label=f"{next_rank['price']:,} tokens",
                    style=discord.ButtonStyle.red,
                    disabled=True,
                )
                async def red_button(
                    self, cant_afford_interaction: discord.Interaction, button: discord.ui.Button
                ):
                    pass
