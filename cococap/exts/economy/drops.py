...
from cococap import entity_models
...
class Drop:
    def __init__(self, channel: discord.TextChannel, amount, ismegadrop):
        ...
        if self.isMegadrop:  # Mega drop, run special init method
            ...
            self.expired_embed.set_footer(
                text="Do /megadrop to see its current status."
            )
    ...
    async def prep_claim(self, drop):
        class ClaimDropButtons(discord.ui.View):
            ...
            @discord.ui.button(
                label="CLAIM", emoji="📦", style=discord.ButtonStyle.green
            )
            async def claim_button(
                self, claim_interaction: discord.Interaction, button: discord.ui.Button
            ):
                user = RequestUser(
                    claim_interaction.user.id, interaction=claim_interaction
                )
                ...
                if self.isMegadrop:
                    ...
                    claimed_embed.add_field(
                        name="Last Winner",
                        value=f"The last winner was {self.megadrop.last_winner}",
                        inline=False,
                    )
                    claimed_embed.set_footer(
                        text=f"The megadrop has been growing since: {self.megadrop.date_started}"
                    )
                    self.megadrop.last_winner = (
                        claim_interaction.user.name
                    )  # last winner is user who claimed it
                    self.megadrop.amount = (
                        self.megadrop.total_drops
                    ) = (
                        self.megadrop.total_drops_missed
                    ) = self.megadrop.times_missed = self.megadrop.COUNTER = 0
                    ...
                    claimed_embed.set_footer(
                        text="Drops happen randomly and last for 30 minutes!"
                    )
                    self.megadrop.total_drops += 1
                    self.megadrop.COUNTER += 1
                ...
                await claim_interaction.response.edit_message(
                    embed=claimed_embed, view=None
                )
                self.claimed = True

        message = await self.channel.send(
            embed=drop.embed, view=ClaimDropButtons(self.isMegadrop)
        )
...
class DropsCog(commands.Cog, name="Drops"):
    ...
    async def drop_task(self):
        ...
        channels = [
            858549045613035541,
            959271607241683044,
            961471869725343834,  # All channels drops can be sent to
            961045401803317299,
            962171274073899038,
            962171351794327562,
        ]
        ...
        self.drop_task.change_interval(
    ...
    async def megadrop_status(self, interaction: discord.Interaction):
        ...
        status_embed.add_field(
            name="Last Winner", value=megadrop.last_winner, inline=True
        )
