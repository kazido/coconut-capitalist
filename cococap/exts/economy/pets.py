...
pet_shop_embed = Cembed(title="Welcome to the pet shop!", desc=desc, color=discord.Color.green())
...
for pet, stats in Pets.select().dicts():
    embeds[pet] = Cembed(
        ...
        color=discord.Color.from_str(stats["color"]),
    )
    emoji_string = ""
    for animal, dictionary in stats["animals"].items():
        emoji_string += f"{dictionary['emoji']} "

    embeds[pet].add_field(name="Possible Pets", value=f"{emoji_string}")
    embeds[pet].add_field(
        name="Stats",
        value=f"Max Level: **{stats['max level']:,}**\n" f"Health: **{stats['health']:,}**",
        inline=False,
    )
    embeds[pet].add_field(
        name="Pet Perks",
        value=f"Work: **+{int(stats['bonuses']['work'] * 100):,}%** bits\n"
        f"Daily: **+{stats['bonuses']['daily']:,}** tokens",
        inline=False,
    )
